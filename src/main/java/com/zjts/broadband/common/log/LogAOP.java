...
public class LogAOP {
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        try {
            ...
            map.put("exceptionCode", null);
            map.put("exceptionDetail", null);
            map.put("params", null);
            map.put("createBy", null);

            Enumeration<String> attributeNames = request.getSession().getAttributeNames();

            map.put("createDate", (int)(new Date().getTime()/1000));
            //保存数据库
            mongoTemplate.save(new BasicDBObject(map));
        } catch (Exception e) {
            //记录本地异常日志
...
//            }
            ...
            /*==========数据库日志=========*/
            Map<String, Object> map = new HashMap();
            map.put("description", getServiceMthodDescription(joinPoint));
            map.put("method", (joinPoint.getTarget().getClass().getName() + "." + joinPoint.getSignature().getName() + "()"));
            map.put("type", 1);
            map.put("requestIp", ip);
            map.put("exceptionCode", e.getClass().getName());
            map.put("exceptionDetail", e.getMessage());
